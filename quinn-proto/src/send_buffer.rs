...
impl SendBuffer {
    ...
    pub fn poll_transmit(&mut self, max_len: usize) -> Range<u64> {
        ...
        // Transmit new data
        let end = self.offset.min(self.unsent + max_len as u64);
        self.in_flight += end - self.unsent;
    ...
    /// Number of bytes that have been sent but are neither acknowledged nor scheduled for retransmit
    /// (i.e. lost)
    ...
    ///
    /// There may still be sent unacknowledged data.
